...
def boundary_attention(x_hf_agg, dim=2304, kernel_shape=3, L_num=3):
    ...
    boundary_att = linear_upsample(boundary_att).permute(0, 3, 1, 2)  # [B, 768, H, W]
...
def process_image(data):
    ...
    # 使用特征融合模块对 feat_L 进行处理
    fusion_module = FeatureFusion(768).to(device)
    z2 = fusion_module(feat_L)
