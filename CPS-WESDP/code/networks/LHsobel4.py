...
class MF(nn.Module):
    def __init__(self, channels):
        ...
        self.final_conv = nn.Conv2d(16, 3, kernel_size=1, bias=False)

    def forward(self, x):
        b, c, h, w = x.shape
        target_h, target_w = h, w
        x_mask = torch.mul(self.mask_map(x).repeat(1, 3, 1, 1), x)
...
def boundary_attention(x_hf_agg, dim=9, kernel_shape=3, L_num=3):
    ...
    sobel_kernel_x = torch.from_numpy(sobel_kernel_x).float()
    sobel_kernel_y = torch.from_numpy(sobel_kernel_y).float()
    ...
    linear_upsample = nn.Linear(dim, 3)  # Output 3 channels
...
def process_image(data):
    ...
    feat_H = F.relu(nn.BatchNorm2d(feat_H.size(1))(feat_H))
    feat_L = F.relu(nn.BatchNorm2d(feat_L.size(1))(feat_L))
    ...
    alpha = 0.5
    z = F.interpolate(alpha * z1 + (1 - alpha) * z2, size=(224, 224), mode='bicubic')
...
def visualize(tensor1, tensor2, title1="Image 1", title2="Image 2"):
    tensor1 = tensor1.squeeze(0).permute(1, 2, 0).clamp(0, 1).detach()
    tensor2 = tensor2.squeeze(0).permute(1, 2, 0).clamp(0, 1).detach()
...
visualize(data, enhanced_image, "Original Image", "Enhanced Image")