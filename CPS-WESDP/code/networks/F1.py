...
# MF class for multi-scale feature aggregation (unchanged)
class MF(nn.Module):
    def __init__(self, channels):
        super(MF, self).__init__()
        self.mask_map = nn.Conv2d(channels, 1, 1, 1, 0, bias=True)
        self.softmax = nn.Softmax(-1)
        self.bottleneck = nn.Conv2d(channels, 16, 3, 1, 1, bias=False)
...
# Boundary attention (simplified for this explanation)
def boundary_attention(x_hf_agg, dim=2304, kernel_shape=3, L_num=3):
    ...
    boundary_lvl_agg = nn.Conv3d(in_channels=dim, out_channels=dim, kernel_size=(L_num, 1, 1), bias=False, groups=dim).to(device)
...
def process_image(data):
    ...
    wave = Wave(768).to(device)  # 将模型移动到设备上
    ...
    mf = MF(768).to(device)  # 将模型移动到设备上
    z2 = mf(feat_L)

    # Apply boundary attention for edge refinement
    z1 = boundary_attention(feat_H)

    # Combine both features, ensuring better spatial resolution
    ...
    # You can add additional loss functions or refine the output as needed.
