...
from dataloaders.dataset1 import VOCSegmentation
from utils.losses import OhemCrossEntropy as CrossEntropyLoss1
from modeling.build_sam import build_sam_vit_b
...
parser.add_argument('-multimask_output', type=int, default=2,
...
parser.add_argument('--base_lr', type=float, default=0.000130,
...
def patients_to_slices(dataset, patiens_num):
    ...
    if "ACDC" in dataset:
        ref_dict = {"3": 68, "7": 136,
                    "14": 256, "21": 396, "28": 512, "35": 664, "140": 7100}
    elif "Prostate":
...
def train(args, snapshot_path):
    ...
    model1 = build_sam_vit_b(args, './checkpoint/sam/sam_vit_b_01ec64.pth').cuda()
    ...
    db_train = VOCSegmentation(args, split='train2')
    db_val = VOCSegmentation(args, split='test3')
    ...
    optimizer1 = optim.AdamW(model1.parameters(), lr=base_lr,
                             betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-6)
    optimizer2 = optim.AdamW(model2.parameters(), lr=base_lr,
                             betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-6)
    ce_loss = CrossEntropyLoss1()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, label_batch, pt, point_labels = sampled_batch['image'], sampled_batch['label'], sampled_batch[
                'pt'], sampled_batch['p_label']
            volume_batch, label_batch, pt, point_labels = volume_batch.cuda(), label_batch.cuda(), pt.cuda(), point_labels.cuda()
            point_coords = pt
            ...
            if args.mod == 'sam_adpt':
                for n, value in model1.image_encoder.named_parameters():
                    if "Adapter" not in n:
                        value.requires_grad = False
                    else:
                        value.requires_grad = True
            imge = model1.image_encoder(volume_batch)
            with torch.no_grad():
                if args.net == 'sam' or args.net == 'mobile_sam':
                    se, de = model1.prompt_encoder(
                        points=pt,
                        boxes=None,
                        masks=None,
                    )
            pred, _ = model1.mask_decoder(
                image_embeddings=imge,
                image_pe=model1.prompt_encoder.get_dense_pe(),
                sparse_prompt_embeddings=se,
                dense_prompt_embeddings=de,
                multimask_output=(args.multimask_output > 1),
            )

            outputs1 = F.interpolate(pred, size=(args.out_size, args.out_size))
            ...
            loss1 = 0.4 * (ce_loss(outputs1[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + 0.6 * dice_loss(
                outputs_soft1[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))
            loss2 = 0.4 * (ce_loss(outputs2[:args.labeled_bs], label_batch[:args.labeled_bs].long()) + 0.6 * dice_loss(
            ...
            if iter_num > 0 and iter_num % 300 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume1(
                        sampled_batch["image"], sampled_batch["label"], sampled_batch['pt'], sampled_batch['p_label'],
                ...
                IOU1 = np.mean(metric_list, axis=0)[0]
                performance1 = np.mean(metric_list, axis=0)[1]
                ...
                logging.info(
                    'iteration %d : model1_mean_IOU : %f model1_mean_dice : %f model1_mean_hd95 : %f' % (
                    iter_num, IOU1, performance1, mean_hd951))
                ...
                IOU2 = np.mean(metric_list, axis=0)[0]
                performance2 = np.mean(metric_list, axis=0)[1]
                ...
                logging.info(
                    'iteration %d : model2_mean_IOU : %f model2_mean_dice : %f model2_mean_hd95 : %f' % (
                        iter_num, IOU2, performance2, mean_hd952))
...
if __name__ == "__main__":
    ...
    '''
    logging.basicConfig(filename=snapshot_path + "/4号脊柱大+大10samsw+noxiaobo+nopre.txt", level=logging.INFO,
